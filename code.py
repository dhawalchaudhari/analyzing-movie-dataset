...
def duplicate_and_unique_movies(dataset, index_):
    ...
    print(f'Number of movies that are duplicated in the list are : {len(duplicatelist)}')
    print(f'Examples : {duplicatelist[0:2]}')
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    """
    rated_movies = []
    rated_movies = [i for i in dataset if float(i[11]) >= float(rate_low) and float(i[11]) <= float(rate_high)]
...
# Create a dict{ionary, 'reviews_max' that will have the name of the movie as key, and the maximum number of reviews as values.
reviews_max = {}
for movie in movies:
    movie_name = movie[13]
    movie_review = float(movie[12])
    
    if movie_name in reviews_max and reviews_max[movie_name] < movie_review:
        reviews_max[movie_name] = movie_review
...
movie_clean = [i for i in movies if (i[13] in reviews_max and float(i[12]) == reviews_max[i[13]])]

# Calling movies_lang(), extract all the english movies and store it in movies_en.
movies_en = movies_lang(movie_clean,3,'en')
